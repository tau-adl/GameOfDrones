...
public abstract class TelloDatagram
{
    #region CRC Tables

    private static readonly ushort[] TBL_CRC16 = {
    ...
    private static readonly byte[] TBL_CRC8 = {
    ...
    protected static byte ComputeCrc8(byte[] buffer, int offset, int count)
    {
        byte seed = 0x77;
        for (var i = 0; i < count; ++i)
            seed = TBL_CRC8[(seed ^ buffer[offset + i]) & 0xff];
        return seed;
    }

    protected static ushort ComputeCrc16(byte[] buffer, int offset, int count)
    {
        ushort seed = 0x3692;
        for (var i = 0; i < count; ++i)
            seed = unchecked((ushort)(TBL_CRC16[(seed ^ buffer[offset + i]) & 0xff] ^ (seed >> 8)));
        return seed;
    }
    ...
    protected abstract byte[] SerializeCore();
